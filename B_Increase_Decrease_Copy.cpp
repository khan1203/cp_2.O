...
int main()
{
    ...
    while (tc--)
    {
        ...
        bool flag = 0;
        ll ops = 0, mx = LLONG_MIN, mn = LLONG_MAX, diff_min = LLONG_MAX;
        for (int i = 0; i < n; i++)
        {
            // if (mx < a[i]) mx = a[i];
            // if (mx < b[i]) mx = b[i];
            ...
            ll diff_n = min(abs(b[n]-a[i]), abs(b[n]-b[i]));
            diff_min = min(diff_min, diff_n);
        ...
        if (flag)
            ...
            ops+= diff_min+1;
            // ll diff_mn = abs(mn-b[n]);
            // ll diff_mx = abs(mx-b[n]);
            // ll diff = min(diff_mn, diff_mx);
            // ops += diff + 1;
        }
